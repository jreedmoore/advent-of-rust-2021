mod puzzle {
  ...
  impl GameState {
    ...
    pub fn new(player_one_pos: u32, player_two_pos: u32) -> GameState {
      GameState {
        player_one: GamePlayer { position: player_one_pos - 1, score: 0 },
        player_two: GamePlayer { position: player_two_pos - 1, score: 0 },
        die_rolls: 0,
        die_state: 0,
  ...
  pub mod parser {
    ...
    use nom::{
      ...
      bytes::complete::{take_until, tag}, character::complete::multispace0, error::ParseError
    };
    ...
    fn starting_position(input: &str) -> IResult<&str, u32> {
      preceded(tuple((take_until(":"), tag(":"))), ws(nom::character::complete::u32))(input)
    }
    ...
    pub fn puzzle_input(input: &str) -> IResult<&str, GameState> {
      map(player_positions, |(player_one, player_two)|  GameState::new(player_one, player_two))(input)
    }
  }
