mod puzzle {
  ...
  impl Point {
    fn parse(input: &str) -> Option<Point> {
      let xs : Vec<u32> = input.split(",").map(|s| s.parse::<u32>().ok()).collect::<Option<Vec<u32>>>()?;
  ...
  impl OrigamiPaper {
    fn parse_points(input: &[&str]) -> Option<OrigamiPaper> {
      let points : HashSet<Point> = input.iter().map(|s| Point::parse(s)).collect::<Option<HashSet<Point>>>()?;
  ...
  impl FoldDirection {
    ...
    fn parse_directions(inputs: &[&str]) -> Option<Vec<FoldDirection>> {
      inputs.iter().map(|s| FoldDirection::parse_direction(s)).collect()
    }
  }

  pub fn parse_input(input: &str) -> Option<(OrigamiPaper, Vec<FoldDirection>)> {
    let (points, directions) : (Vec<&str>, Vec<&str>)= 
      input.lines()
        .map(|s| s.trim())
        .filter(|s| s.len() != 0)
        .partition(|s| !s.contains("fold"));
    Some((OrigamiPaper::parse_points(&points)?,FoldDirection::parse_directions(&directions)?))
  }
