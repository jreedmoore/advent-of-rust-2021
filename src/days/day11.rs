mod puzzle {
  ...
  impl Cave {
    ...
    pub fn get_neighbors(&self, row: u8, col: u8) -> Vec<(u8, u8)> {
      ...
      for r in ir - 1 .. ir + 2  {
        for c in ic - 1 .. ic + 2 {
          if r >= 0 && r < self.width.into() &&
             c >= 0 && c < self.height().into() {
    ...
    pub fn pretty_print(&mut self) -> String {
      (0..self.height()).map(|row| { 
        (0..self.width).map(|col| {
          self.get_at(row as u8, col as u8).energy.to_string()
        }).collect()
  ...
  pub fn parse_input(input: &str) -> Option<Cave> {
    ...
    if lines.len() == 0 {
      ...
      let energy: Option<Vec<u8>> = 
        lines.iter()
          .flat_map(|l| l.chars())
          .map(|c| c.to_string().parse::<u8>().ok())
          .collect();
      energy.map (|es| {
        let octopi: Vec<Octopus> = es.iter().map(|e| Octopus { energy: *e, flashed: false}).collect();
        Cave { octopi : octopi, width : width, flashes : 0}
      })
    }
  }
}
...
mod tests {
  ...
  fn test_neighbors() {
    ...
    assert_eq!(cave.get_neighbors(2, 2), vec![(1,1),(1,2),(1,3),(2,1),(2,3),(3,1),(3,2),(3,3)]);
