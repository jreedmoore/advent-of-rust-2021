...
mod puzzle {
  ...
  impl Probe {
    pub fn with_velocity(velocity: Vec2) -> Probe {
      Probe { velocity: velocity, position: Vec2 { x:0, y:0}}
    }
  ...
  impl Target {
    fn contains_point(&self, point: &Vec2) -> bool {
      ...
      point.x <= self.top_right.x && point.y <= self.top_right.y
    }
  }
  #[cfg(test)]
  mod tests {
    ...
    fn test_target_contains() {
      let target = Target { bottom_left: Vec2 {x: 20, y: -10}, top_right: Vec2 {x:30, y:-5}};
    ...
    fn test_probe_max_height() {
      let target = Target { bottom_left: Vec2 {x: 20, y: -10}, top_right: Vec2 {x:30, y:-5}};
      assert_eq!(Probe::with_velocity(Vec2 { x: 6, y: 9 }).max_height(&target), Some(45)); //maximum height
    }
  }
  ...
  mod parser {
    ...
    pub fn input_parser(input: &str) -> IResult<&str, Target> {
      ...
      let (input, xs) = delimited(tag("x="), range_pair, terminated(tag(","), multispace0))(input)?;
    ...
    mod tests {
      ...
      fn test_parse_targets() {
        assert_eq!(input_parser("target area: x=20..30, y=-10..-5").unwrap().1,
          Target { bottom_left: Vec2 {x: 20, y: -10}, top_right: Vec2 {x:30, y:-5}});
        assert_eq!(input_parser("target area: x=236..262, y=-78..-57").unwrap().1,
          Target { bottom_left: Vec2 {x: 236, y: -78}, top_right: Vec2 {x:262, y:-57}});
      }
    }
  }
