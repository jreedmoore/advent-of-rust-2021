mod puzzle {
    ...
    impl Packet {
        ...
        pub fn evaluate(&self) -> u64 {
            match self {
                ...
                Packet::Op(Operator {
                    type_id: OperatorType::Sum,
                    sub_packets: packets,
                    ..
                }) => packets
                    .iter()
                    .map(|p| Packet::evaluate(p))
                    .fold(0, |acc, b| acc + b),
                Packet::Op(Operator {
                    type_id: OperatorType::Product,
                    sub_packets: packets,
                    ..
                }) => packets
                    .iter()
                ...
                Packet::Op(Operator {
                    type_id: OperatorType::GreaterThan,
                    sub_packets: packets,
                    ..
                }) => {
                    let ps: Vec<u64> = packets.iter().map(|p| Packet::evaluate(p)).collect();
    ...
    mod tests {
        ...
        fn test_parse_literal() {
            ...
            assert_eq!(
                Literal::maybe_parse(&mut HexStringReader::new(hex_string.clone())),
                Some(Literal {
                    version: 6,
                    value: 2021
                })
            ...
            assert_eq!(
                Packet::maybe_parse(&mut HexStringReader::new(hex_string.clone()))
                    .unwrap()
                    .version(),
                6
            );
        ...
        fn test_parse_operator() {
            ...
            let op = Operator::maybe_parse(&mut HexStringReader::new(
                bit_length_operator_string.clone(),
            ))
            .unwrap();
            assert_eq!(op.version, 1);
            assert_eq!(op.sub_packets.len(), 2);

            let op_packet = Packet::maybe_parse(&mut HexStringReader::new(
                bit_length_operator_string.clone(),
            ))
            .unwrap();
            ...
            let op_p =
                Operator::maybe_parse(&mut HexStringReader::new(packet_length_operator_string))
                    .unwrap();
...
pub fn part_one(input: &str) -> Option<u64> {
    ...
    Some(
        packet
            .vec()
            .iter()
            .fold(0u64, |acc, p| acc + p.version() as u64),
    )
}
