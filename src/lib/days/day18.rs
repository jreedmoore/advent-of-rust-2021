mod puzzle {
    ...
    impl SnailfishNum {
        ...
        fn split(&mut self) -> bool {
            for i in 0..self.nums.len() {
                ...
                if v.value >= 10 {
                    self.nums[i] = Elem {
                        value: SnailfishNum::half_up(v.value),
                        depth: v.depth + 1,
                    };
                    self.nums.insert(
                        i,
                        Elem {
                            value: SnailfishNum::half_down(v.value),
                            depth: v.depth + 1,
                        },
                    );
                    return true;
    ...
    mod tests {
        ...
        fn test_num_magnitude() {
            ...
            assert_eq!(
                SnailfishNum::parse("[[9,1],[1,9]]").unwrap().magnitude(),
                129
            );
            assert_eq!(
                SnailfishNum::parse("[[1,2],[[3,4],5]]")
                    .unwrap()
                    .magnitude(),
                143
            );
            assert_eq!(
                SnailfishNum::parse("[[[[0,7],4],[[7,8],[6,0]]],[8,1]]")
                    .unwrap()
                    .magnitude(),
                1384
            );
            assert_eq!(
                SnailfishNum::parse("[[[[1,1],[2,2]],[3,3]],[4,4]]")
                    .unwrap()
                    .magnitude(),
                445
            );
            assert_eq!(
                SnailfishNum::parse("[[[[3,0],[5,3]],[4,4]],[5,5]]")
                    .unwrap()
                    .magnitude(),
                791
            );
            assert_eq!(
                SnailfishNum::parse("[[[[5,0],[7,4]],[5,5]],[6,6]]")
                    .unwrap()
                    .magnitude(),
                1137
            );
            assert_eq!(
                SnailfishNum::parse("[[[[8,7],[7,7]],[[8,6],[7,7]]],[[[0,7],[6,6]],[8,7]]]")
                    .unwrap()
                    .magnitude(),
                3488
            );
