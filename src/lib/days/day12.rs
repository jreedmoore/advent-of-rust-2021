...
mod puzzle {
    ...
    impl CaveType {
        fn parse(input: &str) -> Option<CaveType> {
            match input {
                ...
                big if big.chars().all(|c| char::is_uppercase(c)) => {
                    Some(CaveType::Big(big.to_string()))
                }
                small if small.chars().all(|c| char::is_lowercase(c)) => {
                    Some(CaveType::Small(small.to_string()))
                }
                _ => None,
            }
        }
    ...
    pub fn parse_input(input: &str) -> Option<CaveSystem> {
        let connections: Option<Vec<Connection>> = input
            .lines()
            .map(|l| l.trim())
            .filter(|l| l.len() != 0)
