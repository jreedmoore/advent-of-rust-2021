...
mod tests {
    ...
    #[test]
    fn test_from_data() {
        test_valid_access(Grid::from_data(
            3,
            vec![
    ...
    fn test_neighbors() {
        ...
        assert_eq!(
            grid.orthog_neighbors(1, 1),
            vec![(0, 1), (1, 0), (1, 2), (2, 1),]
        );

        assert_eq!(grid.diag_neighbors(0, 0), vec![(0, 1), (1, 0), (1, 1),]);

        assert_eq!(grid.diag_neighbors(2, 2), vec![(1, 1), (1, 2), (2, 1),]);
    ...
    #[test]
    fn test_offsets() {
        let grid: Grid<bool> = Grid::from_data(
            3,
            vec![false, true, false, true, false, true, false, true, false],
        );

        assert_eq!(
            grid.diag_offsets(1, 1),
            vec![
                (0, 0),
                (0, 1),
                (0, 2),
        ...
        assert_eq!(
            grid.diag_offsets(0, 0)
                .into_iter()
                .map(|(r, c)| grid.get_int(r, c).map(|rf| rf.clone()))
                .collect_vec(),
            vec![
                None,
                None,
